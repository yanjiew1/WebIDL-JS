...
export const generateAsyncIteratorArgConversions = (ctx, idl, parent, errPrefix) => {
  const requires = new utils.RequiresMap(ctx);
  let str = "const args = [];";
...
export const generateOverloadConversions = function (ctx, typeOfOp, name, parent, errPrefix) {
  const requires = new utils.RequiresMap(ctx);
